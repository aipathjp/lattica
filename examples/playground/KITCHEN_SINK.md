...
that exercises a broad slice of the Taible public API as a *consumer* (it only
imports the published `@ai-path/tb-*` packages and never reaches into their `src`).
...
- **`useGridController` + `<LatticaGrid>`** — the headless model wired to the
  canvas-rendered, virtualized grid view (`@ai-path/tb-react`).
...
| Export CSV | `serializeDelimited(...)` from `@ai-path/tb-io` |
| Export XLSX | `matrixToXlsx(...)` from `@ai-path/tb-io` |

## AI panel

A button runs `nlToFormula` against an `AIClient` backed by a deterministic
`MockProvider` (all from `@ai-path/tb-ai`). `nlToFormula` calls `generateObject`,
...
pnpm --filter @ai-path/tb-example-playground run typecheck
```

The page typechecks against the **built** `@ai-path/tb-*` packages (`packages/*/dist`).
If you change a package, rebuild it first so its `dist` type declarations are
current. `@ai-path/tb-ai` was added to the playground's `dependencies` for this demo.
