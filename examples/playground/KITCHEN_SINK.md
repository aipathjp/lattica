...
imports the published `@ai-path/lattica-*` packages and never reaches into their `src`).
...
- **`useGridController` + `<LatticaGrid>`** — the headless model wired to the
  canvas-rendered, virtualized grid view (`@ai-path/lattica-react`).
...
| Export CSV | `serializeDelimited(...)` from `@ai-path/lattica-io` |
| Export XLSX | `matrixToXlsx(...)` from `@ai-path/lattica-io` |

## AI panel

A button runs `nlToFormula` against an `AIClient` backed by a deterministic
`MockProvider` (all from `@ai-path/lattica-ai`). `nlToFormula` calls `generateObject`,
...
pnpm --filter @ai-path/lattica-example-playground run typecheck
```

The page typechecks against the **built** `@ai-path/lattica-*` packages (`packages/*/dist`).
If you change a package, rebuild it first so its `dist` type declarations are
current. `@ai-path/lattica-ai` was added to the playground's `dependencies` for this demo.
