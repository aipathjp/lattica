...
Import/export and clipboard interop for Taible, with no runtime dependencies.
...
```ts
import { parseDelimited, serializeDelimited, parseTsv } from '@ai-path/tb-io';
...
```ts
import { toClipboardText, toClipboardHtml, parseClipboard } from '@ai-path/tb-io';
...
```ts
import { writeXlsx, matrixToXlsx, type XlsxWorkbook } from '@ai-path/tb-io';
...
```ts
import { readXlsx, type ReadWorkbook } from '@ai-path/tb-io';
